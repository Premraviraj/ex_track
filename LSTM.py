...
class FinancialAnalyzer:
    ...
    def plot_savings(self, savings, freq='W'):
        ...
        try:
            ...
            if len(weekly_savings) < 2 or len(monthly_savings) < 2:
                logger.error("Not enough data for LSTM predictions")
                # Create a simple plot without predictions
                plt.figure(figsize=(12, 6))
                plt.plot(weekly_savings.index, weekly_savings.values, 'b-', label='Weekly Savings')
                plt.plot(monthly_savings.index, monthly_savings.values, 'g-', label='Monthly Savings')
                plt.title('Savings Trends')
                plt.xlabel('Date')
                plt.ylabel('Amount (₹)')
                plt.legend()
                plt.grid(True)
            ...
            # Create and train models with reduced epochs
            weekly_model = self._create_lstm_model()
            monthly_model = self._create_lstm_model()
            
            # Train models with fewer epochs and larger batch size
            weekly_model.fit(
            ...
            # Make predictions
            weekly_pred = weekly_model.predict(self._prepare_data(weekly_data)[0], verbose=0)
            monthly_pred = monthly_model.predict(self._prepare_data(monthly_data)[0], verbose=0)
            ...
            # Save plot to bytes with optimized settings
            buf = BytesIO()
            plt.savefig(buf, format='png', bbox_inches='tight', dpi=100)  # Reduced DPI for faster saving
            plt.close()
            buf.seek(0)
    ...
    def _create_lstm_model(self):
        """Create and return a simplified LSTM model"""
        try:
            model = Sequential([
                LSTM(32, activation='relu', input_shape=(1, 1)),  # Simplified architecture
                Dense(1)
            ])
